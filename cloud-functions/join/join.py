...
def get_backend_core_counts() -> Dict[str, BackendCoreCount]:
    return {"c2-standard-16": BackendCoreCount(total=3, frontend=1, drive=1)}


service = build('compute', 'v1')


def get_join_params(project: str, zone: str, username: str, password: str, tag: str):
    res = service.instances().list(project=project, zone=zone).execute()
    items = [item for item in res['items'] if tag in item['tags'].get('items', [])]
    backend_ips = [item['networkInterfaces'][0]['networkIP'] for item in items]
    instance_type = items[0]['machineType'].split('/')[-1]

    bash_script_template = r"""
#!/bin/bash
...
"""
    ...
    return bash_script_template % (
        username, password, " ".join(backend_ips), instance_params.total, instance_params.frontend,
        instance_params.drive, ",".join(backend_ips))
...
def join(request):
    ...
    tag = get_param(request_json, request_args, 'tag')
    if not tag:
        return ''

    return get_join_params(project=project, zone=zone, username=username, password=password, tag=tag)
